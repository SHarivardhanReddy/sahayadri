...
import os
# Prefer the large generated CSV; generate if missing
CSV_PATH = 'data/workers_health_10000.csv'
if not os.path.exists(CSV_PATH):
    print(f"{CSV_PATH} not found — generating dataset (this may take a few seconds)...")
    # generate dataset programmatically
    from scripts.generate_large_dataset import NUM_ROWS, OUT_CSV
    # running the generator will create the file
    import subprocess, sys
    subprocess.check_call([sys.executable, 'scripts/generate_large_dataset.py'])
...
# Features and label
X = pd.concat([df[['age'] + binary_cols], work_dummies], axis=1)
